...
class TreatmentImageIntegrationTest {

    @Autowired private lateinit var mockMvc: MockMvc
    @Autowired private lateinit var serviceRepository: TreatmentRepository
    @Autowired private lateinit var serviceImageRepository: ServiceImageRepository
    ...
    fun setup() {
        ...
        serviceImageRepository.deleteAll()
    ...
    fun `POST image - owner uploads image and gets 200 with imageUrl`() {
        val service = serviceRepository.save(
            ProvidedService(companyId = companyId, name = "Strzyżenie", durationMinutes = 30, price = 80)
        )
        val file = MockMultipartFile("image", "photo.jpg", "image/jpeg", ByteArray(1024))

        mockMvc.perform(
            multipart("/api/services/${service.id}/image")
                .file(file)
    ...
    fun `POST image - employee gets 403`() {
        val service = serviceRepository.save(
            ProvidedService(companyId = companyId, name = "Masaż", durationMinutes = 60, price = 120)
        )
    ...
    fun `POST image - unauthenticated gets 403`() {
        ...
        mockMvc.perform(
            multipart("/api/services/${service.id}/image").file(file)
        )
            .andExpect(status().isForbidden)
    }

    @Test
    fun `POST image - unknown service returns 404`() {
        val file = MockMultipartFile("image", "photo.jpg", "image/jpeg", ByteArray(100))

        mockMvc.perform(
            multipart("/api/services/99999/image")
                .file(file)
                .header("Authorization", "Bearer ${ownerToken()}")
        )
    ...
    fun `POST image - exceeding 5 image limit returns 409`() {
        ...
        repeat(5) { i ->
            serviceImageRepository.save(ServiceImage(serviceId = service.id!!, imageUrl = "http://example.com/img$i.jpg"))
        }
    ...
    fun `DELETE image - owner deletes image and response has empty images list`() {
        ...
        val image = serviceImageRepository.save(
            ServiceImage(serviceId = service.id!!, imageUrl = "http://pub.r2.dev/services/1/1/abc.jpg")
        )

        mockMvc.delete("/api/services/${service.id}/image/${image.id}") {
            header("Authorization", "Bearer ${ownerToken()}")
        }.andExpect {
            status { isOk() }
    ...
    fun `DELETE image - unknown image returns 404`() {
        ...
        mockMvc.delete("/api/services/${service.id}/image/99999") {
            header("Authorization", "Bearer ${ownerToken()}")
        }.andExpect {
            status { isNotFound() }
        }
